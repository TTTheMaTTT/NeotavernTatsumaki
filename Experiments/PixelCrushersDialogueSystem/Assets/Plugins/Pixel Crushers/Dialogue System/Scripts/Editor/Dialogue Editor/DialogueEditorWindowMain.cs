...
namespace PixelCrushers.DialogueSystem.DialogueEditor
{
    ...
    public partial class DialogueEditorWindow : EditorWindow
    {
        ...
        // Referencies on actors in all conversations in the current database.
        // It is dictionary with actorId as key. It has dictionary as value. This dictionary has conversationId as key and number of actor referencies as value.
        [SerializeField]
        private Dictionary<int, Dictionary<int, int>> actorsReferencies = new Dictionary<int, Dictionary<int, int>>();
        ...
        private void CalculateActorsReferencies()
        {
            if( database == null ) {
                actorsReferencies = new Dictionary<int, Dictionary<int, int>>();
                return;
            }
            foreach( var conversation in database.conversations ) {
                foreach( var dialogueEntry in conversation.dialogueEntries ) {
                    ...
                    foreach( var actorState in dialogueEntry.actorsStates ) {
                        ...
                        if( !actorsReferencies[actorState.ActorID].ContainsKey( conversation.id ) ) {
                            actorsReferencies[actorState.ActorID].Add( conversation.id, 0 );
                        }
                        actorsReferencies[actorState.ActorID][conversation.id]++;
                    }
                }
            }
        }
