...
namespace PixelCrushers.DialogueSystem
{
    ...
    public static class Tools
    {
        ...
        /// </summary>
        /// <typeparam name="T">Parsed enum type</typeparam>
        /// <param name="enumString">The string</param>
        /// <param name="value">Result value</param>
        /// <param name="ignoreCase"></param>
        /// <returns>true if conversion is successful</returns>
        public static bool StringToEnum<T>( string enumString, ref T value, bool ignoreCase = true )
        {
            try {
        ...
        private const string RichCodePattern = @"<b>|</b>|<i>|</i>|<color=[#]?\w+>|</color>";

        public static string StripRichTextCodes(string s)
        {
            if (!s.Contains("<")) return s;
            return Regex.Replace(s, RichCodePattern, string.Empty);
        }

        public static bool IsRichTextCode( string s )
        {
            Match match = Regex.Match( s, RichCodePattern );
            return match.Success && match.Length == s.Length;
        }

        private const string TextMeshProTagPattern =
            @"<b>|</b>|<i>|</i>|<color=[#]?\w+>|<color=""\w+"">|</color>|" +
        ...
        {
            if (!s.Contains("<")) return s;
            return Regex.Replace(s, TextMeshProTagPattern, string.Empty);
        }

        public static bool IsTextMeshProTag( string s )
        {
            Match match = Regex.Match( s, TextMeshProTagPattern );
            return match.Success && match.Length == s.Length;
        }

        // doesn't treat "\<" and "\>" as tag borders
        private const string TagPattern = @"(?<!\\)<[^<>]*(?<!\\)>";

        // Replaces "\<" with "<" and "\>" with ">" because in result string there are no tag borders to misinterpret with
        public static string UnescapeTagBrackets( string s)
        {
            return s.Replace( "\\<", "<" ).Replace( "\\>", ">" );
        }

        // Removes all  tags. 
        // Also replaces "\<" with "<" and "\>" with ">" because in result string there are no tag borders to misinterpret with
        public static string StripTags( string s )
        {
            if( !s.Contains( "<" ) )
                return s;
            return Regex.Replace( s, TagPattern, string.Empty ).Replace( "\\<", "<" ).Replace( "\\>", ">" );
        }

        // Return all xml-kind tag borders from string
        public static MatchCollection GetTagMatches( string s )
        {
            return Regex.Matches( s, TagPattern );
        }
